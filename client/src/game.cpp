...
Game::Game() :
    ...
    correctionInterval(0.01f),
    nameInputActive(false),
    selectedColorIndex(0), // 50ms
    chatInputLength(0),
    chatInputActive(false) // 10ms
{
...
void Game::init(int width, int height, const std::string& title) {
    ...
    // Set default color
    playerManager->getLocalPlayer().color = ColorUtils::getColorFromIndex(
        selectedColorIndex);
    ...
    network->setPlayerListCallback(
        [this](const std::vector<PlayerInfo>& playerList) {
            playerManager->updatePlayers(playerList, network->getPlayerId());
        });

    network->setPositionCallback(
        [this](const std::string& playerId, float x, float y) {
            playerManager->processPositionUpdate(playerId, x, y,
                                                 network->getPlayerId());
        });
...
void Game::render() {
    ...
    case GameState::INPUT_NAME:
        uiManager->drawNameInputScreen(nameInput, nameLength, nameInputActive,
                                       selectedColorIndex,
                                       ColorUtils::availableColors,
                                       colorButtons,
                                       playerManager->getLocalPlayer());
        break;

    case GameState::CONNECTING:
        uiManager->drawConnectingScreen(
            network ? network->getStatusMessage() : "Connecting...");
        break;

    case GameState::PLAYING:
        uiManager->drawGameScreen(playerManager->getLocalPlayer(),
                                  playerManager->getPlayers(),
                                  nameInput, chatMessages,
                                  network->getChatMessages(),
                                  chatInputActive, chatInput,
                                  uiManager->getChatInputBox());
        break;

    case GameState::DISCONNECTED:
        uiManager->drawDisconnectedScreen(
            network ? network->getStatusMessage() : "Unknown error");
        break;
    }
    ...
    case GameState::INPUT_NAME:
        ...
        if (nameInputActive) {
            ...
            if (IsKeyPressed(KEY_ENTER) && nameLength > 0) {
                ...
                std::string colorString = ColorUtils::ColorToString(
                    playerManager->getLocalPlayer().color);
                ...
                if (network && network->connect(serverAddress, tcpPort, udpPort)) {
                    ...
                    std::cout << "Connection initiated, moving to CONNECTING state" <<
                        std::endl;
                }
                else {
                    state = GameState::DISCONNECTED;
...
void Game::sendPlayerUpdate() {
    if (network && network->isConnected()
        && playerManager->getLocalPlayer().initialPositionReceived) {
        network->sendPositionUpdate(
            playerManager->getLocalPlayer().x,
            playerManager->getLocalPlayer().y
        );
    }
}
